...
with open('research/indigenous_entries_en.json', encoding='utf-8') as f:
    entries = json.load(f)

INSTRUCTION = """You are translating a Canadian-made product field guide entry from English to Canadian French (the register used on official Government of Canada bilingual pages, e.g. canada.ca French pages). Translate each of these fields naturally and accurately, preserving brand names, proper nouns, Indigenous nation names, and place names unchanged:
- category
- product_or_line
- common_use
...
Do NOT translate brand names, Indigenous nation/community names, HS heading numbers, or URLs. If a source field is an empty string, return an empty string for its translation too. Use formal but accessible French suitable for a public-facing government-style bilingual website. Preserve respectful, accurate language when referring to First Nations, Inuit, and Metis peoples and communities."""
...
SECTOR_FR = {
    "Hand tools, hardware and building products": "Outils à main, quincaillerie et produits de construction",
    "Pantry, bakery, snacks, beverages, frozen and canned foods": "Produits d’épicerie, de boulangerie, collations, boissons, aliments surgelés et en conserve",
    "Clothing, footwear and textiles": "Vêtements, chaussures et textiles",
    "Beer, wine and spirits": "Bière, vin et spiritueux",
    "Cleaning, beauty and personal care": "Entretien ménager, beauté et soins personnels",
}
...
for entry, result in zip(entries, results, strict=True):
    ...
    entry["sector_fr"] = SECTOR_FR[entry["sector"]]
    output.append(entry)
...
with open('research/indigenous_entries_bilingual.json', 'w', encoding='utf-8') as f:
